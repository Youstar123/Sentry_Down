...
//static float LQR_k[3][12]=
//        {
////                {-14.5517, -0.9657, -0.0497, -1.2720, 8.6148, 1.1493, 25.1661, 1.5247, 0.1265, 3.2271, 15.0261, 1.2431}, // K BOT 160 [100 1 500 100 5000 1]
//                {-9.8483, -0.8506, -1.0837, -1.5961, 7.7766, 1.2499, 17.5565, 1.3521, 2.9120, 3.9418, 12.7236, 1.0783}, // K BOT 160 [60 40 450 1 5000 1]
...
//static float LQR_k[3][12]=
//        {
//                {-12.5200,-0.7275, -0.5125, -0.9081,6.8146,1.1860,30.2244, 2.4959, 3.9706, 6.4980, 4.4412,0.1307}, // K BOT 160 [100 1 500 100 5000 1]
//                {-12.8744, -1.3209, -1.1211, -1.7588, 5.9249, 1.0449, 18.0335, 1.5066, 1.9438, 2.7914, 14.7395, 1.6264}, // K BOT 240 [60 40 450 1 5000 1]
//                {-15.1573, -1.7839, -1.1963, -1.8960, 4.9468, 0.9224, 17.3352, 1.5197, 1.4588, 2.1008, 17.0028, 2.0054}, // K BOT 320
//        }; // Ts=1.5ms Q=diag([1 1 100 500 5000 1]) R=[240 0;0 25]
...
static void chassis_kf_update(void)
{
    ...
    chassis_kf[LEFT].MeasuredVector[0] = lk_motor[LEFT]->measure.total_angle * WHEEL_RADIUS;//x
    chassis_kf[LEFT].MeasuredVector[1] = imu_motion_velocity[LEFT] + leg[LEFT]->d_phi0*leg[LEFT]->PendulumLength + ins.gyro[2] * 0.25f;//速度
    chassis_kf[LEFT].MeasuredVector[2] = lk_motor[LEFT]->measure.speed_rads * WHEEL_RADIUS;//速度
...
static void update_LQR_obs() {
    ...
    chassis_x=LQRXObsBuf[LEFT][2];
    if(chassis_x){

    }
    LQRXObsBuf[LEFT][3] = chassis_kf[LEFT].FilteredValue[1];
    chassis_v=LQRXObsBuf[LEFT][3];
    if(chassis_v){
...
//    pid_config_t theta_pid_config = INIT_PID_CONFIG(40, 0, 1, 0, 20, PID_Integral_Limit);
    pid_config_t theta_pid_config = INIT_PID_CONFIG(24, 0, 5, 0, 20, PID_Integral_Limit);
...
//    pid_config_t yaw_pid_config = INIT_PID_CONFIG(0.0002, 0, 0.0002, 0, 1, PID_Integral_Limit); // P: 0.004
    pid_config_t yaw_pid_config = INIT_PID_CONFIG(0.0001, 0, 0.0003, 0, 2, PID_Integral_Limit); // P: 0.004
    /* 跟随云台参数 */
